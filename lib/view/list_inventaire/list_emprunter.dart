...
class ListEmprunter extends ConsumerWidget {
  const ListEmprunter({super.key});
            ...
            dispoAsync.when(
              data: (empruter) {
                ...
                if (recherche.isEmpty) {
                  ...
                  listEmprunter = empruter
                      .where(
                        (e) => e.etatInventaire == EtatInventaire.emprunter,
                      )
                      .toList();
                } else {
                ...
                return Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        "${listEmprunter.length}/${empruter.length}",
                        style: TextStyle(
                          fontSize: 20,
                      ...
                      Expanded(
                        child: ListView.builder(
                          itemCount: listEmprunter.length,
                          itemBuilder: (context, index) {
                            final emprunts = listEmprunter[index];
                            return containerList.buildCard(
                              context,
                              emprunts.name,
                              emprunts.description,
                              Couleur.couleurEtat(emprunts.etatInventaire),
                              inventaire: emprunts,
                              ref: ref,
                            );
                          },
